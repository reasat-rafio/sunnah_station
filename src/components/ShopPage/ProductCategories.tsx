...
import { ChangeCategoryVariants } from "../../../utils/motion/animation";
...
export const ProductCategories: React.FC<ProductCategoriesProps> = ({}) => {
  ...
  const [ShowMoreMensWearings, SetShowMoreMensWearings] =
    useState<boolean>(false);
  const [SowMoreWomensWearings, SetSowMoreWomensWearings] =
    useState<boolean>(false);
  const [showMoreHomeAppliances, SetShowMoreHomeAppliances] =
    useState<boolean>(false);
          ...
          {state && (
            ...
            >
              {sub_category.map((sub_c, i) => (
                <Link href={`/shop/${main_category}/${sub_c.sub_link}`} key={i}>
                  <li
                    key={i}
                    className={`hover:text-lightBlue cursor-pointer my-2 ${
                      sub_categories == sub_c.sub_link && "text-lightBlue"
                    }`}
  ...
  const outPutHomeAppliances = (name: string, state: any, sub_category) => {
    return (
      name === "home-appliances" && state && sub_ctg(name, sub_category, state)
    );
  };
      ...
      <ul className=" ">
        {Categories.map(({ category_name, sub_category, link }, i) => (
          <li key={i} className="my-4 text-gray-600">
            ...
            {sub_category &&
              outputMesnWearings(link, ShowMoreMensWearings, sub_category)}
            {sub_category &&
              outputWomensWearings(link, SowMoreWomensWearings, sub_category)}
            {sub_category &&
              outPutHomeAppliances(link, showMoreHomeAppliances, sub_category)}
          </li>
        ))}
      </ul>
